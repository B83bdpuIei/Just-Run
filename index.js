...
// Se eliminó la primera llamada a client.login() aquí.

// Se eliminaron las líneas de depuración duplicadas y se consolidaron en un solo bloque.

// === BLOQUE DE DIAGNÓSTICO FINAL (CORREGIDO) ===
console.log('--- INICIANDO DIAGNÓSTICO FINAL ---');
const botToken = process.env.DISCORD_TOKEN; // Usamos un solo nombre de variable
if (botToken) {
    console.log(`Token encontrado. Longitud: ${botToken.length}. Inicio: '${botToken.substring(0, 5)}...'. Final: '...${botToken.substring(botToken.length - 5)}'.`);
} else {
    console.log('¡ERROR! La variable process.env.DISCORD_TOKEN está vacía o no existe.');
}
console.log('--- FIN DEL DIAGNÓSTICO ---');

// === Llamada ÚNICA y CORRECTA para el login ===
