...
        } else if (commandName === 'add_user_compo') {
            // CAMBIO: Se ha simplificado la lógica para que no duplique líneas.
            try {
                await interaction.deferReply({ flags: [MessageFlags.Ephemeral] });
...
                }
                
                // Se ha modificado esta parte para añadir al usuario sin preguntar por el rol si no es un puesto "X"
                const lineaOriginal = originalLines.find(linea => linea.startsWith(`${puestoAAgregar}.`));
                const lineaActual = lineas[lineaNuevaIndex];
                
                let nuevoValor;
                if (lineaActual.includes('. X')) {
                    // Si el puesto es genérico "X", se pide un rol
...
                    });
                    ...
                    // Si el puesto ya tiene un rol predefinido, simplemente se añade al usuario
