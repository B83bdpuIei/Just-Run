    ...
    if (interaction.isChatInputCommand()) {
        ...
        if (commandName === 'start_comp') {
            ...
            try {
                ...
                // Restaura el puesto anterior si el usuario ya está en la lista
                const originalContent = originalCompoContent.get(mensajePrincipal.id);
                if (!originalContent) {
                ...
                // Lógica de corrección: determina si el puesto tiene un rol predefinido
                const originalLineForNewSpot = originalLines.find(linea => linea.startsWith(`${puestoAAgregar}.`));
                
                if (originalLineForNewSpot && !originalLineForNewSpot.includes('. X')) {
                    // El puesto tiene un rol definido, lo añadimos directamente
                    const nuevoValor = `${originalLineForNewSpot} <@${usuarioAAgregar.id}>`;
                    const lineasDeContenido = originalContent.split('\n');
                    const offset = lineasDeContenido.findIndex(linea => linea.startsWith(`${puestoAAgregar}.`));
                    const inicioPartyIndex = lineas.findIndex(linea => linea.startsWith('1.'));

                    lineas[inicioPartyIndex + offset] = nuevoValor;

                    await mensajePrincipal.edit(lineas.join('\n'));
                    await interaction.editReply(`✅ Usuario <@${usuarioAAgregar.id}> añadido al puesto **${puestoAAgregar}** como **${originalLineForNewSpot.split('.')[1].trim()}**.`);
                } else {
                    // El puesto es genérico (X), así que pedimos el rol.
                    await interaction.editReply({ content: `Has apuntado a <@${usuarioAAgregar.id}> en el puesto **${puestoAAgregar}**. Escribe el rol en el chat.`});
                    ...
                    colector.on('collect', async m => {
                        ...
                        await mensajePrincipal.edit(lineas.join('\n'));
                        await hilo.send(`✅ Usuario <@${usuarioAAgregar.id}> añadido al puesto **${puestoAAgregar}** como **${rol}**.`);
                        colector.stop();
                    });

                    colector.on('end', collected => {
                        if (collected.size === 0) {
                            hilo.send(`🚫 No respondiste a tiempo. El usuario <@${usuarioAAgregar.id}> no ha sido añadido.`);
                        }
                    });
                }
            } catch (error) {
        ...
        if (interaction.customId.startsWith('start_comp_modal_')) {
            ...
            try {
                ...
                // AÑADIDO: Guardar el contenido original de la plantilla en el Map
